...
def estimate_geometry(
    ...
    cfg = GeometricVerificationConfig(**config) if isinstance(config, dict) else (config or GeometricVerificationConfig())
    ...
    if target_model == TransformModel.AUTO:
        if total_pts >= 4:
            # Try the least flexible model first.  Equal inlier support should
            # prefer the simpler physically interpretable transform instead of
            # unnecessarily fitting an affine/projective model.
            models_to_evaluate = [TransformModel.SIMILARITY, TransformModel.AFFINE, TransformModel.HOMOGRAPHY]
        elif total_pts >= 3:
